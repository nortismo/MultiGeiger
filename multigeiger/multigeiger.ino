...
#define TIME_TO_SLEEP 60000000 * 30  //sleep for 30 minutes
...
void loop() {
  ...
  transmit(current_ms, gm_counts, gm_count_timestamp, hv_pulses, have_thp, temperature, humidity, pressure, ST_WIFI_OFF);
  ...
  if (goToSleep){
      goToSleep = false;
      log(INFO, "Going to deep-sleep for %d minutes", uint64_t(uint64_t(TIME_TO_SLEEP) / 60000000));
